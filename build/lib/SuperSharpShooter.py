...
import base64, gzip, random, string, sys, argparse, re, os
...
"""
    ...
    def read_file(x, f):
        # Load packaged templates regardless of current working dir
        if f.startswith("templates/"):
            name = f.split("/", 1)[1]
            try:
                with ir.open_text("templates", name, encoding='utf-8') as fs:
                    return fs.read()
            except Exception as e:
                print_bad(f"Template not found: {f}", e)
...
End Sub"""
        ...
        #print(payload)
        if(x.payload_type == 8):
            f.write(x.macro_stager.encode('utf-8'))  # FIXED
        if(x.payload_type == 9):
